...
fn bench_tiny_parallel(b: &mut Bencher) {
    let tiny = &["a"];
    b.iter(|| parallel_letter_frequency::frequency_strategy(tiny, 3));
}
...
fn bench_small_parallel(b: &mut Bencher) {
    ...
    b.iter(|| parallel_letter_frequency::frequency_strategy(&texts, 3));
}
...
#[bench]
fn bench_one_text_parallel(b: &mut Bencher) {
    let text = one_text(90);
    b.iter(|| parallel_letter_frequency::frequency_strategy(&[text.as_str() ], 3));
}

#[bench]
fn bench_one_text_sequential(b: &mut Bencher) {
    let text = one_text(90);
    b.iter(|| frequency(&[text.as_str()]));
}

/// Simple sequential char frequency. Can it be beat?
pub fn frequency_simple(texts: &[&str]) -> HashMap<char, usize> {
...
pub fn frequency(texts: &[&str]) -> HashMap<char, usize> {
    //frequency_simple(texts)
    parallel_letter_frequency::frequency_seq(texts)
}

fn one_text(repeat: usize) -> String {
    [ODE_AN_DIE_FREUDE, WILHELMUS, STAR_SPANGLED_BANNER]
        .iter()
        .cycle()
        .take(3 * repeat)
        .flatten().flat_map(|text| text.chars()).collect()
}
