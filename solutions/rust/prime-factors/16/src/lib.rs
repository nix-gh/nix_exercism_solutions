...
fn factors_from_no_vec(n: u64, from: u64) -> Box<dyn Iterator<Item = (u64, u32)>> {
    if n < from {
    ...
    lazy_prime_generator(from, n)
        ...
        .map(|f| {
            ...
            Box::new(iter::once((f, exp)).chain(factors_from_no_vec(n / f.pow(exp), f + 1)))
        })
        .unwrap()
}

pub fn factors_rec_no_vec(n: u64) -> Vec<u64> {
    factors_from_no_vec(n, 2)
        .flat_map(|(factor, factor_count)| iter::repeat_n(factor, factor_count as usize))
        .collect()
}
